...
function getTrendDiffKeyPoints(list: [number, ...string[]][], scanArea = 1): [number, [number, ...string[]]][] {
  ...
  for (let i = 0; i < listWithDelta.length; i++) {
    ...
    if (
      ((i >= scanArea) && (i + scanArea <= maxIdx))
      &&
      currentDeltaTrend !== preDeltaTrend
      &&
      scanAreaRangeList.every(offset => getTrend(listWithDelta[(i - offset)][12]) === preDeltaTrend)
      &&
      scanAreaRangeList.every(offset => getTrend(listWithDelta[(i + offset)][12]) === currentDeltaTrend)
    ) {
...
function getStrictTrendDiffKeyPoints(list: [number, ...string[]][]): [number, ...string[]][] {
  ...
  return listWithDelta
    .reduce((acc, item, idx) => {
      ...
      return acc;
    }, [[listWithDelta[0][12] >= 0, [list[0]]]] as [boolean, [number, ...string[]][]][])
...
export function generateSpeedColorGradient(list: [number, ...string[]][]): ColorGradientInfo {
  ...
  const extremePoints = getStrictTrendDiffKeyPoints(trendDiffKeyPoints.slice(1, -1).map(([, item]) => item))
    .map(item => {
      const speedDistancePercent = (+item[7] - maxSpeed) / speedDistance;
      const color = maxSpeedColor.rotate(speedDistancePercent * MAX_COLOR_DEGREES | 0).hex();
      return [...item, color]
...
export function generateSpeedColorGradient1(list: [number, ...string[]][]) {
  ...
  const colorGradientInfo = colorList.reduce((acc, item, idx) => {
  ...
  let preDegree = 0;
  const markers = colorGradientInfo.pointInfoData.reduce((acc, item) => {
    const currentDegree = utils.getNearestValue(+item[14], [0, -60, -120]);
    ...
    return acc;
  }, [] as { degree: number, group: (string | number)[][] }[]).map(({ degree, group }) => {
    const nearestValue = utils.getNearestValue(degree, group.map(item => +item[14]));
    ...
    .reduce((acc, { 7: speed }, idx) => {
      if (!idx) {
        ...
        if (Math.abs(speedDistance) > MAX_SPEED_SHAKE_IGNORE) {
          ...
          if ((prevColor === BrakingColor.RED && speedDistance < 0) || (prevColor === BrakingColor.GREEN && speedDistance > 0)) {
            // 直接更新进度
